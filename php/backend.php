...
if($_POST['request'] == 'data') {  
	...
	$value 			=	array(
							array('string', 'Mock_test_db.mock_test_tbl', 'name', 'Name as name, Id as id, Email as email, Phone as phoneNum, Gender as gender','id'),
							array('email', 'Mock_test_db.mock_test_tbl', 'email', 'Name as name, Id as id, Email as email, Phone as phoneNum, Gender as gender','id')
						);
	$query_data 	=	array();
	...
	if($tble1=='') {
		...
		$response_data['table_heading_name']	=	$table_heading_name;
		$response_data['table_column_name']		=	$table_column_name;
		$response_data['get_query_and_data']	=	$get_query_and_data;
		echo json_encode($response_data);
	}
}
...
Class ManageApp {
	...
	function get_data($connection_mock_chat,$data_from,$data_to,$data_per_page,$where) {   
		...
		$params			=	array();
		$max_page		=	'';
		$queryOne		=	"SELECT COUNT(*)as total_row FROM mock_test_tbl WHERE ".$where;
		$total_row		=	mysqli_prepared_query($connection_mock_chat,$queryOne,"",$params);
		...
		if($extra_slots_entry) {
			foreach ($extra_slots_entry as $val) {
				...
				$res_here['queryOne']		=	$queryOne;
				$res_here['query']			=	$query;
				$response[]					=	$res_here;
			}
		} 
		return $response;
