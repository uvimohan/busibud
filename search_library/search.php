...
class searching {   
                ...
                if ($value_q['type'] == 'string') {  
                    if(!empty($expert_and_company)) {
                        ...
                        foreach ($expert_and_company as $key => $value) {
                            ...
                            // array_push($attachment, '('.$value_q['search_col_name'].' LIKE "%'.$value.'%" || email LIKE "%'.$value.'%")');
                        }
    ...
    function searching_data($ids_of_string){
        ...
        if (preg_match_all($pattern_date, $this->input, $matches_out)) {
            ...
            if($length<=5) {
                $month  =   ($new_date[0] > 12) ? $new_date[0] : "";
                $Date1  =   ($new_date[1] > 31) ? $new_date[1] : "";  
                $year   =   '2019';
            } else {
                $year   =   $new_date[2];
                $month  =   ($new_date[1] > 12) ? $new_date[1] : "";
                $Date1  =   ($new_date[0] > 31) ? $new_date[0] : "";
